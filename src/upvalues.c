...
static ObjectUpvalue* capture_upvalue(ObjectUpvalue *head, Value *stack_slot);
static void           close_upvalue(ObjectUpvalue *head, Value *stack_top); 
...
static void close_upvalue(ObjectUpvalue *head, Value *stack_slot){
   ...
   while(head != NULL && head->location >= stack_slot){
      ObjectUpvalue *current = head;
